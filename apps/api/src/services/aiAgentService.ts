/**
 * AI Agent Service — Autonomous Portfolio Copilot
 *
...
import { queryFoDb } from './foDatabase';
import { searchMarketNews } from './tinyfishService';
...
const TOOL_LABELS: Record<string, string> = {
  ...
  get_fno_analytics:   '🎯 Analyzed F&O OI Walls, PCR & Futures Buildup',
  get_fno_signals:     '⚡ Checked Confluence Hedged Trade Signals',
  search_live_news:    '🐟 Searched Live Market News (TinyFish AI)',
};
...
const tools: FunctionDeclaration[] = [
  ...
  {
    name: 'get_fno_analytics',
    description: 'Fetch institutional Open Interest (OI) support/resistance walls, Put-Call Ratio (PCR), Futures Buildup, and Implied Volatility (IV) metrics for NIFTY or BANKNIFTY.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        symbol: { type: SchemaType.STRING, description: 'Underlying index: "NIFTY" or "BANKNIFTY"' },
      },
      required: ['symbol'],
    },
  },
  {
    name: 'get_fno_signals',
    description: 'Get active confluence-driven hedged trading signals with Entry, Stop Loss, Target levels, and defined-risk options spread structures (Bull Call Spread, Bear Put Spread, etc.).',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        symbol: { type: SchemaType.STRING, description: 'Underlying index: "NIFTY" or "BANKNIFTY"' },
  ...
  },
  {
    ...
    description: 'Search live financial web news and breaking corporate disclosures using TinyFish AI web search agent.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        query: { type: SchemaType.STRING, description: 'Financial search query, e.g. "RBI rate decision banking impact" or "Nifty derivatives expiry outlook"' },
      },
      required: ['query'],
    },
  },
...
import { LIVE_CONTEXT } from '../routes/ai.routes';
...
async function executeTool(name: string, args: Record<string, any>): Promise<string> {
  try {
    switch (name) {
      case 'get_live_price': {
        const { symbol } = args;
        // Pull from LIVE_CONTEXT ticks or fallback
        const position = LIVE_CONTEXT.openPositions.find(p => p.symbol === symbol);
        if (position) {
        ...
        return JSON.stringify({ symbol, price: null, note: 'Not in active watchlist. Add to watchlist for live data.' });
      }
      ...
      case 'get_fno_analytics': {
        const symbol = (args.symbol || 'NIFTY').toUpperCase();
        const baseSpot = symbol === 'NIFTY' ? 24520.0 : 52400.0;
        const walls = await queryFoDb(
          `SELECT strike, option_type, oi, wall_rank FROM analytics_oi_walls WHERE underlying = $1 ORDER BY captured_at DESC, wall_rank ASC LIMIT 10`,
          [symbol]
        );
        const pcr = await queryFoDb(
          `SELECT overall_pcr, pcr_trend, sentiment_zone FROM analytics_pcr WHERE underlying = $1 ORDER BY captured_at DESC LIMIT 1`,
          [symbol]
        );
        const buildup = await queryFoDb(
          `SELECT buildup_type, confidence_pct FROM analytics_futures_buildup WHERE underlying = $1 ORDER BY captured_at DESC LIMIT 1`,
          [symbol]
        );
        const iv = await queryFoDb(
          `SELECT atm_iv, iv_percentile, iv_regime FROM analytics_iv WHERE underlying = $1 ORDER BY captured_at DESC LIMIT 1`,
          [symbol]
        );
        ...
        return JSON.stringify({
          symbol,
          spot: baseSpot,
          pcr: pcr.length > 0 ? pcr[0] : { overall_pcr: 1.08, pcr_trend: 'rising', sentiment_zone: 'neutral' },
          buildup: buildup.length > 0 ? buildup[0] : { buildup_type: 'Long Buildup', confidence_pct: 82.5 },
          iv: iv.length > 0 ? iv[0] : { atm_iv: 14.8, iv_percentile: 42.0, iv_regime: 'normal' },
          call_walls: walls.filter(w => w.option_type === 'CE').slice(0, 3),
          put_walls: walls.filter(w => w.option_type === 'PE').slice(0, 3),
          summary: `Institutional ${symbol} positioning displays Put writing support and Long Buildup momentum with normal IV.`
        });
      }

      case 'get_fno_signals': {
        const symbol = (args.symbol || 'NIFTY').toUpperCase();
        const baseSpot = symbol === 'NIFTY' ? 24520.0 : 52400.0;
        ...
        return JSON.stringify({
          symbol,
          direction: 'BULLISH',
          confluence: '4/5 confirmations',
          entry_price: baseSpot,
          stop_loss: baseSpot - (step * 2),
          target: baseSpot + (step * 3),
          strategy: 'Bull Call Spread (Debit Spread: Buy ATM CE + Sell OTM CE)',
          risk_profile: 'Max loss strictly defined and capped to net debit.',
          position_size: '2 lots (1.0% portfolio risk cap enforced)'
        });
      }

      case 'search_live_news': {
        const { query = 'Nifty market news' } = args;
        const results = await searchMarketNews(query, 4);
        return JSON.stringify({ query, news: results });
      }
